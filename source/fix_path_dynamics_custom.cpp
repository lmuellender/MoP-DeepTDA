...
/* ----------------------------------------------------------------------
MODIFIED VERSION to perform MetaD of Paths for a single particle in Mueller-Brown potential.
see spring_force()
------------------------------------------------------------------------- */
...
FixPATH_DYNAMICS::FixPATH_DYNAMICS(LAMMPS *lmp, int narg, char **arg) : Fix(lmp, narg, arg), id_pe(NULL), pe(NULL)
{
  ...
  // fix <fix-ID> all path_dynamics <dt> <damp> <eps> [keyword <values> ...]
  if (narg < 6)
    error->universe_all(FLERR, "Illegal fix path_dynamics command");
  ...
  if (dt_path <= 0.0)
    error->universe_all(FLERR, "Fix path_dynamics dt_path must be > 0");
  if (damp_path <= 0.0)
    error->universe_all(FLERR, "Fix path_dynamics damp_path must be > 0");
  if (epsilon <= 0.0)
    error->universe_all(FLERR, "Fix path_dynamics epsilon must be > 0");
  ...
  if (narg > iarg)
  {
    if (strcmp(arg[iarg], "langevin") == 0)
    {
      ...
      if (target_temp <= 0.0)
        error->universe_all(FLERR, "Fix path_dynamics temp must be > 0");
      if (lange_tau <= 0.0)
        error->universe_all(FLERR, "Fix path_dynamics tau must be > 0");
      if (seed <= 0)
        error->universe_all(FLERR, "Fix path_dynamics seed must be > 0");
      iarg += 3;
    }
    else if (strcmp(arg[iarg], "nhc") == 0)
    {
      ...
      if (target_temp <= 0.0)
        error->universe_all(FLERR, "Fix path_dynamics temp must be > 0");
      if (nhc_nchain <= 0)
        error->universe_all(FLERR, "Fix path_dynamics nchain must be > 0");
      iarg += 2;
    }
...
void FixPATH_DYNAMICS::init()
{
  ...
  if (icompute < 0)
    error->universe_all(FLERR, "Potential energy ID for fix pathmd does not exist");
  ...
  for (int i = 1; i <= atom->ntypes; i++)
    fmass[i] = atom->mass[i]; // Room for improvement here
...
{
  if (me_u == 0 && screen)
    fprintf(screen, "Setting up Classical Path Dynamics\n");
  if (me_u == 0 && screen && (run == NVE))
    fprintf(screen, "NVE run\n");
  if (me_u == 0 && screen && (run == DAMPED))
    fprintf(screen, "Damped dynamics\n");
...
void FixPATH_DYNAMICS::post_force(int flag)
{
  ...
  if ((run == DAMPED) || (((run == NEB) || (run == NEB_TEST)) && (min_style == MIN_DAMPED)))
  { // Damped dynamics
...
void FixPATH_DYNAMICS::nhc_init()
{
  ...
  for (int i = 0; i < max; i++)
  {
    ...
    for (int ichain = 1; ichain < nhc_nchain; ichain++)
      nhc_eta_dotdot[i][ichain] = (nhc_eta_mass[i][ichain - 1] * nhc_eta_dot[i][ichain - 1] * nhc_eta_dot[i][ichain - 1] * force->mvv2e - KT) / nhc_eta_mass[i][ichain];
  }
...
void FixPATH_DYNAMICS::nhc_update_v()
{
  ...
  for (int i = 0; i < nmax; i++)
  {
    ...
    for (int ichain = 1; ichain < nhc_nchain; ichain++)
    {
      expfac = exp(-dt8 * eta_dot[ichain + 1]);
      eta_dot[ichain] *= expfac;
      eta_dotdot[ichain] = (nhc_eta_mass[i][ichain - 1] * eta_dot[ichain - 1] * eta_dot[ichain - 1] - KT) / nhc_eta_mass[i][ichain];
      eta_dot[ichain] += eta_dotdot[ichain] * dt4;
      eta_dot[ichain] *= expfac;
    }
...
void FixPATH_DYNAMICS::neb_test_force()
{
  ...
  if ((ibead == nbeads - 1) && (update->ntimestep == 0))
    EFinalIni = veng;
  if ((ibead == 0) && (update->ntimestep == 0))
    EIniIni = veng;
  ...
  for (int i = 0; i < nlocal; i++)
    SpringFPerp[i] = new double[3];
  tangent = new double *[nlocal];
  for (int i = 0; i < nlocal; i++)
    tangent[i] = new double[3];
  eta_l = new double *[nlocal];
  for (int i = 0; i < nlocal; i++)
    eta_l[i] = new double[3];
  eta_n = new double *[nlocal];
  ...
  for (int i = 0; i < nbeads; i++)
    aux_plen[i] = plenall[i] = 0.0;
  aux_plen[ibead] = plen; // Length of this segment
  if (me_w == 0)
    MPI_Allreduce(aux_plen, plenall, nbeads, MPI_DOUBLE, MPI_SUM, rootworld); // Gather all segments, store in array
  MPI_Bcast(plenall, nbeads, MPI_DOUBLE, 0, world);                           // Communicate to all procs
  for (int i = 0; i < ibead + 1; i++)
    lenuntilIm += plenall[i]; // (ibead+1) goes from 1 to nbeads -> i goes from 0 to up to nbeads-1
  for (int i = 0; i < nbeads; i++)
    lentot += plenall[i];
  ...
  // Update forces
  if ((ibead == 0) || (ibead == nbeads - 1))
  { // Just relax to local minimum
    for (int i = 0; i < nlocal; i++)
    {
      f[i][0] = f_bead[i][0];
      f[i][1] = f_bead[i][1];
  ...
  if ((ibead == 0) && FreeEndIni)
  { // Keep energy close to starting point
  ...
  if ((ibead == nbeads - 1) && FreeEndFinal)
  { // Keep energy close to starting point
    double prefactor = 0.0;
    if (veng < EFinalIni)
    {
  ...
  if ((ibead == nbeads - 1) && FreeEndFinalWithRespToEIni)
  { // Keep energy above that of first image
    double prefactor = 0.0;
    if (veng < vIni)
    {
      if (dot < 0)
    ...
    for (int i = 0; i < nsend; i++)
    {
      ...
      if (index < 0)
      {
        ...
        sprintf(error_line, "Atom " TAGINT_FORMAT " is missing at world [%d] "
                            "rank [%d] required by  rank [%d] (" TAGINT_FORMAT ", " TAGINT_FORMAT ", " TAGINT_FORMAT ").\n",
                tag_send[i],
...
void FixPATH_DYNAMICS::compute_force()
{
  ...
  {
    if (force->bond)
      force->bond->compute(0, 0);
    if (force->angle)
      force->angle->compute(0, 0);
    if (force->dihedral)
      force->dihedral->compute(0, 0);
    if (force->improper)
      force->improper->compute(0, 0);
  }
  if (kspace_compute_flag)
    force->kspace->compute(0, 0);
  if (n_pre_reverse)
    modify->pre_reverse(0, 0);
  if (force->newton)
    comm->reverse_comm();
