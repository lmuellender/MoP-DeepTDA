...
FixStyle(path_dynamics_mb,FixPATH_DYNAMICS_MB)

#else

#ifndef FIX_PATH_DYNAMICS_MB_H
#define FIX_PATH_DYNAMICS_MB_H

#include "fix.h"

namespace LAMMPS_NS {

class FixPATH_DYNAMICS_MB : public Fix {
 public:
  FixPATH_DYNAMICS_MB(class LAMMPS *, int, char **);
  //virtual ~FixPATH_DYNAMICS_MB();
  ...
  // --- Muller-Brown coefficients, potential, force & hessian ---
  static const double A_mb[4];
  static const double a_mb[4];
  static const double b_mb[4];
  static const double c_mb[4];
  static const double x0_mb[4];
  static const double y0_mb[4];

  void mb_potential(double *x_b, double &pot);
  void mb_force(double *x_b, double *f);
  void mb_hessian(double *x_b, double *hess);
  ...
  double **H_bead, **f_last;   // MB Hessian, last force
  double **eta_l, **eta_n;
