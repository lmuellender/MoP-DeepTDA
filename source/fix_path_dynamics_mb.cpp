...
#include "domain.h"
#include "error.h"
#include "fix_path_dynamics_mb.h"
#include "force.h"
#include "improper.h"
#include "kspace.h"
...
FixPATH_DYNAMICS_MB::FixPATH_DYNAMICS_MB(LAMMPS *lmp, int narg, char **arg) :
    Fix(lmp, narg, arg), id_pe(NULL), pe(NULL)
{
  ...
  if (narg > iarg) {
    if (strcmp(arg[iarg], "langevin") == 0) {
      ...
      if (iarg + 2 > narg) error->universe_all(FLERR, "Illegal fix path_dynamics command");
      run = NEB;
      if (strcmp(arg[iarg + 1], "sd") == 0) { min_style = MIN_SD; }
      if (strcmp(arg[iarg + 1], "asd") == 0) { min_style = MIN_ASD; }
      if (strcmp(arg[iarg + 1], "damped") == 0) { min_style = MIN_DAMPED; }
      iarg += 1;
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg], "parallel") == 0) {
      ...
      if ((climber < 1) || (climber > universe->nworlds))
        error->universe_all(FLERR, "Illegal fix neb command");
      climber -= 1;
      iarg += 2;
  ...
  vector_flag =
      1;    // 0/1 if compute_vector() function exists ---> accessible as f_{fixid}[j], j=1,size_vector
  ...
  double **v = atom->v;
  if ((run == NEB) || (run == NEB_TEST) || (run == DAMPED))
    for (int i = 0; i < atom->nlocal; i++) v[i][0] = v[i][1] = v[i][2] = 0.0;
...
/*
FixPATH_DYNAMICS_MB::~FixPATH_DYNAMICS_MB()
{
...
int FixPATH_DYNAMICS_MB::setmask()
{
  int mask = 0;
  mask |= POST_FORCE;
...
void FixPATH_DYNAMICS_MB::init()
{
  ...
  if (me_u == 0 && screen)
    fprintf(screen, "Fix path_dynamics initializing Classical Path Dynamics\n");
...
void FixPATH_DYNAMICS_MB::setup(int vflag)
{
  ...
  if (me_u == 0 && screen && (run == LANGEVIN))
    fprintf(screen, "NVT run using Langevin thermostat\n");
  if (me_u == 0 && screen && (run == NHC))
    fprintf(screen, "NVT run using Nose-Hoover chains thermostat\n");
  if (me_u == 0 && screen && (run == NEB)) {
    if (min_style == MIN_SD)
      fprintf(screen, "NEB-like action minimization using steepest descent\n");
    if (min_style == MIN_ASD)
      fprintf(screen, "NEB-like action minimization using accellerated steepest descent\n");
    if (min_style == MIN_DAMPED)
      fprintf(screen, "NEB-like action minimization using damped dynamics\n");
  }
...
void FixPATH_DYNAMICS_MB::initial_integrate(int /*vflag*/)
{
  //Half time step in v
  if (run == NHC)
    nhc_update_v();
  else if ((run == NEB) || (run == NEB_TEST))
    neb_update_v();
  else
    update_v();
  //Full time-step in x
...
void FixPATH_DYNAMICS_MB::final_integrate()
{
...
void FixPATH_DYNAMICS_MB::post_force(int flag)
{
  ...
  memcpy(buf_force[x_last], buf_beads[x_last],
         sizeof(double) * atom->nlocal * 3);    //store forces in buf_force
  memcpy(buf_force[x_next], buf_beads[x_next],
         sizeof(double) * atom->nlocal * 3);    //store forces in buf_force
  comm_exec(atom->x);    //Communicates positions and stores them in buf_beads

  if (run == NEB)
    neb_force();
  else if (run == NEB_TEST)
    neb_test_force();
  else
    spring_force();
  ...
  if ((run == DAMPED) ||
      (((run == NEB) || (run == NEB_TEST)) && (min_style == MIN_DAMPED))) {    //Damped dynamics
...
void FixPATH_DYNAMICS_MB::nhc_init()
{
...
void FixPATH_DYNAMICS_MB::update_x()
{
...
void FixPATH_DYNAMICS_MB::neb_update_v()
{
  int nlocal = atom->nlocal;
  int *type = atom->type;
  double **v = atom->v;
  ...
  if (min_style == MIN_ASD) {
    ...
    fdotf = fdotfall;
    if (vdotfall >
        0.0)    // v_new = dot_product(v_old,f)*f/|f|**2 + f*dt; accellerated steepest descent
    {
...
void FixPATH_DYNAMICS_MB::update_v()
{
  int nlocal = atom->nlocal;
  int *type = atom->type;
  double **v = atom->v;
...
void FixPATH_DYNAMICS_MB::nhc_update_v()
{
  int n = atom->nlocal;
  int *type = atom->type;
  double **v = atom->v;
...
void FixPATH_DYNAMICS_MB::spring_force()
{
  spring_energy = 0.0;
  potential_energy = 0.0;
  ...
  //Potential energy (Muller-Brown)
  mb_potential(x[0], potential_energy);

  double maxeta2 = 0.0;
  
  // std::cout << "current position of bead "<<ibead<<": "<<x[0][0]<<" "<<x[0][1]<<std::endl;
  ...
  f_bead = new double* [nlocal];    // MB force acting on current bead
  for (int i=0;i<nlocal;i++) f_bead[i] = new double [3];
  f_last = new double* [nlocal];    // MB force acting on last bead
  for (int i=0; i<nlocal; i++) f_last[i] = new double[3];
  H_bead = new double* [nlocal];    // MB Hessian on current bead
  for (int i=0; i<nlocal; i++) H_bead[i] = new double[4];

  // ATTACHING DEBUGGER
  // volatile int i = 0;
  // char hostname[256];
  // gethostname(hostname, sizeof(hostname));
  // printf("Bead %d: PID %d on %s ready for attach\n", ibead, getpid(), hostname);
  // fflush(stdout);
  // while (0 == i) {
  //   sleep(5);
  // }

  // special case: Nbeads=1
  if (nbeads == 1) {
    mb_force(x[0], f_bead[0]);
    f[0][0] = f_bead[0][0];
    f[0][1] = f_bead[0][1];
    f[0][2] = f_bead[0][2];
    // std::cout << "MB force on single particle: " << f[0][0] << " " << f[0][1] << std::endl;
  }
  else if (ibead == 0)    //first bead
  {
    for (int i = 0; i < nlocal; i++) {
      ...
      mb_force(x[i], f_bead[i]);    // calculate force due to MB potential
      // std::cout << "MB force on bead "<<ibead<<": "<< f_bead[i][0]<<" "<<f_bead[i][1] <<std::endl;
      eta_n[i][0] = dx - dt_md[type[i]] * f_bead[i][0] * force->ftm2v;
      eta_n[i][1] = dy - dt_md[type[i]] * f_bead[i][1] * force->ftm2v;
      eta_n[i][2] = 0.;
      // std::cout<<"eta_n on bead "<<ibead<<": "<<eta_n[i][0]<<" "<<eta_n[i][1]<<std::endl;
      xnext += 3;
      double msd =
          (eta_n[i][0] * eta_n[i][0] + eta_n[i][1] * eta_n[i][1] + eta_n[i][2] * eta_n[i][2]);
    ...
    for (int i = 0; i < nlocal; i++) {    //f = force-field + ~spring~ "*" ~hessian~
      mb_hessian(x[i], H_bead[i]);
      // std::cout << "hessian on bead "<<ibead<<": "<<H_bead[i][0]<<" "<<H_bead[i][1]<<" "<<H_bead[i][2]<<" "<<H_bead[i][3]<<std::endl;
      f[i][0] = f_bead[i][0] + md_2dt[type[i]] * force->mvv2e * (
        eta_n[i][0] * (1 + dt_md[type[i]] * H_bead[i][0]) +
        eta_n[i][1] * dt_md[type[i]] * H_bead[i][1]);
      f[i][1] = f_bead[i][1] + md_2dt[type[i]] * force->mvv2e * (
        eta_n[i][0] * dt_md[type[i]] * H_bead[i][2] +
        eta_n[i][1] * (1 + dt_md[type[i]] * H_bead[i][3])); 
      f[i][2] = 0.;
      // std::cout << "eff. force on bead "<<ibead<<": "<< f[i][0]<<" "<<f[i][1] <<std::endl;
    }
  } else if (ibead == nbeads - 1)    
  {
    //last bead: no contrib to spring_energy to avoid double counting, f is only due to last spring.
    ...
    for (int i = 0; i < nlocal; i++) {
      ...
      dz = 0.;
      domain->minimum_image(dx, dy, dz);
      mb_force(xlast, f_last[i]);
      eta_l[i][0] = dx - dt_md[type[i]] * f_last[i][0] * force->ftm2v;
      eta_l[i][1] = dy - dt_md[type[i]] * f_last[i][1] * force->ftm2v;
      eta_l[i][2] = 0.;
      xlast += 3;
      // std::cout<<"eta_l on bead "<<ibead<<": "<<eta_l[i][0]<<" "<<eta_l[i][1]<<std::endl;
    }
    // Update f
  ...
  } else    //bulk beads
  {
    ...
    for (int i = 0; i < nlocal; i++) {    //f = ~springs~ + ~hessian~
      mb_hessian(x[i], H_bead[i]);
      f[i][0] = md_2dt[type[i]] * force->mvv2e * (
        eta_n[i][0] * (1 + dt_md[type[i]] * H_bead[i][0]) +
        eta_n[i][1] * dt_md[type[i]] * H_bead[i][1] -
        eta_l[i][0]);
      f[i][1] = md_2dt[type[i]] * force->mvv2e * (
        eta_n[i][0] * dt_md[type[i]] * H_bead[i][2] +
        eta_n[i][1] * (1 + dt_md[type[i]] * H_bead[i][3]) -
        eta_l[i][1]); 
      f[i][2] = 0.;
  ...
  if (H_bead) {
    for (int i=0; i<nlocal; i++) {
      if (H_bead[i]) delete[] H_bead[i];
    }
    delete[] H_bead;
  }
  if (f_last) {
    for (int i = 0; i < nlocal; i++) {
      if (f_last[i]) delete[] f_last[i];
    }
    delete[] f_last;
  }
  if (f_bead) {
    for (int i = 0; i < nlocal; i++) {
      if (f_bead[i]) delete[] f_bead[i];
    }
    delete[] f_bead;
  }
  if (eta_n) {
    for (int i = 0; i < nlocal; i++) {
      if (eta_n[i]) delete[] eta_n[i];
    }
    delete[] eta_n;
  }
  if (eta_l) {
    for (int i = 0; i < nlocal; i++) {
      if (eta_l[i]) delete[] eta_l[i];
    }
    delete[] eta_l;
  }
}    //END spring_force
...
------------------------------------------------------------------------- */
const double FixPATH_DYNAMICS_MB::A_mb[4] = {-20., -10., -17., 1.5};
const double FixPATH_DYNAMICS_MB::a_mb[4] = {-1., -1., -6.5, 0.7};
const double FixPATH_DYNAMICS_MB::b_mb[4] = {0., 0., 11., 0.6};
const double FixPATH_DYNAMICS_MB::c_mb[4] = {-10., -10., -6.5, 0.7};
const double FixPATH_DYNAMICS_MB::x0_mb[4] = {1., 0., -0.5, -1.};
const double FixPATH_DYNAMICS_MB::y0_mb[4] = {0., 0.5, 1.5, 1.};

void FixPATH_DYNAMICS_MB::mb_potential(double *x_b, double &pot)
{
  double arg = 0.;
  double dx = 0.;
  double dy = 0.;
  ...
  for (int i=0; i<4; i++) {
    dx = x_b[0] - x0_mb[i];
    dy = x_b[1] - y0_mb[i];
    arg = a_mb[i]*dx*dx + b_mb[i]*dx*dy + c_mb[i]*dy*dy;
    pot += A_mb[i] * exp(arg);
  }
}

void FixPATH_DYNAMICS_MB::mb_force(double *x_b, double *f)
{
  double arg = 0.;
  double dx = 0.;
  double dy = 0.;
  ...
  for (int i=0; i<4; i++) {
    ...
    dy = x_b[1] - y0_mb[i];
    arg = a_mb[i]*dx*dx + b_mb[i]*dx*dy + c_mb[i]*dy*dy;
    prefx = 2*a_mb[i]*dx + b_mb[i]*dy;
    prefy = b_mb[i]*dx + 2*c_mb[i]*dy;
    f[0] -= A_mb[i]*prefx*exp(arg);
    f[1] -= A_mb[i]*prefy*exp(arg);
  }
}

void FixPATH_DYNAMICS_MB::mb_hessian(double *x_b, double *hess)
{
  // - hess = - (H_xx, H_xy, H_yx, H_yy)
  double arg = 0.;
  double dx = 0.;
  double dy = 0.;
  double prefx = 0.;
  ...
  for (int i=0; i<4; i++) {
    ...
    prefx = 2*a_mb[i]*dx + b_mb[i]*dy;
    prefy = b_mb[i]*dx + 2*c_mb[i]*dy;
    hess[0] -= A_mb[i] * (2*a_mb[i]+prefx*prefx) * exp(arg);
    hess[1] -= A_mb[i] * (b_mb[i]+prefx*prefy) * exp(arg);
    hess[2] -= A_mb[i] * (b_mb[i]+prefx*prefy) * exp(arg);
    hess[3] -= A_mb[i] * (2*c_mb[i]+prefy*prefy) * exp(arg);
  }
}
...
void FixPATH_DYNAMICS_MB::neb_test_force()
{
  spring_energy = 0.0;
  ...
  if (ibead > 0)
    procprev = universe->root_proc[ibead - 1];
  else
    procprev = -1;

  if (ibead < nbeads - 1)
    procnext = universe->root_proc[ibead + 1];
  else
    procnext = -1;
  ...
  {    //To held it fixed use "partition yes 1 fix set_force 0 0 0" in the input, after fix path_dynamics.
    for (int i = 0; i < nlocal; i++) {
      ...
      dot += f_bead[i][0] * tangent[i][0] + f_bead[i][1] * tangent[i][1] +
          f_bead[i][2] * tangent[i][2];
      //NEB
      double msd =
          eta_n[i][0] * eta_n[i][0] + eta_n[i][1] * eta_n[i][1] + eta_n[i][2] * eta_n[i][2];
      if (ParaSpring)
        spring_energy += 0.5 * kspringPara * msd * force->mvv2e;
      else
        spring_energy += 0.5 * md_2dt[type[i]] * msd * force->mvv2e;
      xnext += 3;
    }
  ...
  if (me_w == 0)
    MPI_Allreduce(aux_plen, plenall, nbeads, MPI_DOUBLE, MPI_SUM,
                  rootworld);                          //Gather all segments, store in array
  MPI_Bcast(plenall, nbeads, MPI_DOUBLE, 0, world);    //Communicate to all procs
  for (int i = 0; i < ibead + 1; i++)
    lenuntilIm +=
        plenall[i];    // (ibead+1) goes from 1 to nbeads -> i goes from 0 to up to nbeads-1
  ...
  if ((ibead == nbeads - 1) &&
      FreeEndFinalWithRespToEIni) {    //Keep energy above that of first image
    double prefactor = 0.0;
    if (veng < vIni) {
      if (dot < 0)
  ...
  if ((ibead > 0) && (ibead < nbeads - 1)) {    //BULK BEADS
    ...
    wgt = 0.5 *
        (1 +
         cos(MY_PI *
             dotpath));    // weight to keep part of perpendicular spring force in case of kinks

    double dotSprTan = 0.0;    // dot_product(spring_force,tangent)
    for (int i = 0; i < nlocal; i++)
      dotSprTan += f[i][0] * tangent[i][0] + f[i][1] * tangent[i][1] + f[i][2] * tangent[i][2];
    double aux = 0.0;
    ...
    if (PerpSpring && (kspringPerp > 0.0)) {
      for (int i = 0; i < nlocal; i++)
        dotSprTanPerp += SpringFPerp[i][0] * tangent[i][0] + SpringFPerp[i][1] * tangent[i][1] +
            SpringFPerp[i][2] * tangent[i][2];
      double aux1 = 0.0;
      ...
      if (PerpSpring && (kspringPerp == 0.0)) {
        for (int i = 0; i < nlocal; i++) {
          f[i][0] = f_bead[i][0] + pref * tangent[i][0] + dotSprTan * tangent[i][0] +
              wgt * (f[i][0] - dotSprTan * tangent[i][0]);
          f[i][1] = f_bead[i][1] + pref * tangent[i][1] + dotSprTan * tangent[i][1] +
              wgt * (f[i][1] - dotSprTan * tangent[i][1]);
          f[i][2] = f_bead[i][2] + pref * tangent[i][2] + dotSprTan * tangent[i][2] +
              wgt * (f[i][2] - dotSprTan * tangent[i][2]);
        }
      } else if (PerpSpring && (kspringPerp > 0.0)) {
        for (int i = 0; i < nlocal; i++) {
          f[i][0] = f_bead[i][0] + pref * tangent[i][0] + dotSprTan * tangent[i][0] +
              wgt * (SpringFPerp[i][0] - dotSprTanPerp * tangent[i][0]);
          f[i][1] = f_bead[i][1] + pref * tangent[i][1] + dotSprTan * tangent[i][1] +
              wgt * (SpringFPerp[i][1] - dotSprTanPerp * tangent[i][1]);
          f[i][2] = f_bead[i][2] + pref * tangent[i][2] + dotSprTan * tangent[i][2] +
              wgt * (SpringFPerp[i][2] - dotSprTanPerp * tangent[i][2]);
        }
      } else {
        for (int i = 0; i < nlocal; i++) {
          f[i][0] = f_bead[i][0] + pref * tangent[i][0] + dotSprTan * tangent[i][0];
  ...
  if (SpringFPerp) {
    for (int i = 0; i < nlocal; i++) {
      if (SpringFPerp[i]) delete[] SpringFPerp[i];
    }
    delete[] SpringFPerp;
  }
  if (tangent) {
    for (int i = 0; i < nlocal; i++) {
      if (tangent[i]) delete[] tangent[i];
    }
    delete[] tangent;
  }
  if (f_fwd) {
    for (int i = 0; i < nlocal; i++) {
      if (f_fwd[i]) delete[] f_fwd[i];
    }
    delete[] f_fwd;
  }
  if (f_bwd) {
    for (int i = 0; i < nlocal; i++) {
      if (f_bwd[i]) delete[] f_bwd[i];
    }
    delete[] f_bwd;
  }
  if (x_bead) {
    for (int i = 0; i < nlocal; i++) {
      if (x_bead[i]) delete[] x_bead[i];
    }
    delete[] x_bead;
  }
  if (f_bead) {
...
void FixPATH_DYNAMICS_MB::neb_force()
{
  spring_energy = 0.0;
  ...
  } else    // bulk beads: I need to compute also the spring forces
  {
    ...
    for (int i = 0; i < nlocal; i++) {    //f = ~springs~ + ~hessian~
      if (ParaSpring) {
        f[i][0] = kspringPara * (eta_n[i][0] - eta_l[i][0]) * force->mvv2e +
            0.25 * kspringPara * (f_fwd[i][0] - f_bwd[i][0]) / eps / md_2dt[type[i]];
        f[i][1] = kspringPara * (eta_n[i][1] - eta_l[i][1]) * force->mvv2e +
            0.25 * kspringPara * (f_fwd[i][1] - f_bwd[i][1]) / eps / md_2dt[type[i]];
        f[i][2] = kspringPara * (eta_n[i][2] - eta_l[i][2]) * force->mvv2e +
            0.25 * kspringPara * (f_fwd[i][2] - f_bwd[i][2]) / eps / md_2dt[type[i]];
      } else {
        f[i][0] = md_2dt[type[i]] * (eta_n[i][0] - eta_l[i][0]) * force->mvv2e +
            0.25 * (f_fwd[i][0] - f_bwd[i][0]) / eps;
        f[i][1] = md_2dt[type[i]] * (eta_n[i][1] - eta_l[i][1]) * force->mvv2e +
            0.25 * (f_fwd[i][1] - f_bwd[i][1]) / eps;
        f[i][2] = md_2dt[type[i]] * (eta_n[i][2] - eta_l[i][2]) * force->mvv2e +
            0.25 * (f_fwd[i][2] - f_bwd[i][2]) / eps;
      }
      if (PerpSpring) {
        SpringFPerp[i][0] = kspringPerp * (eta_n[i][0] - eta_l[i][0]) * force->mvv2e +
            0.25 * kspringPerp * (f_fwd[i][0] - f_bwd[i][0]) / eps / md_2dt[type[i]];
        SpringFPerp[i][1] = kspringPerp * (eta_n[i][1] - eta_l[i][1]) * force->mvv2e +
            0.25 * kspringPerp * (f_fwd[i][1] - f_bwd[i][1]) / eps / md_2dt[type[i]];
        SpringFPerp[i][2] = kspringPerp * (eta_n[i][2] - eta_l[i][2]) * force->mvv2e +
            0.25 * kspringPerp * (f_fwd[i][2] - f_bwd[i][2]) / eps / md_2dt[type[i]];
      }
    }
  }
  //At this point, f_bead[][] contains the forces due to the force-field, f[][] contains the spring forces and optionally SpringFPerp contains the spring forces used only in perp direction
...
void FixPATH_DYNAMICS_MB::comm_init()
{
  if (size_plan) {
    delete[] plan_send;
    delete[] plan_recv;
  ...
  if (buf_force) {
    for (int i = 0; i < size_plan; i++)
      if (buf_force[i]) delete[] buf_force[i];
    delete[] buf_force;
  }
...
void FixPATH_DYNAMICS_MB::comm_exec(double **ptr)
{
  int nlocal = atom->nlocal;
  ...
  if (nlocal > max_nlocal) {
    ...
    buf_recv = (double *) memory->srealloc(buf_recv, size, "FixPATH_DYNAMICS_MB:x_recv");

    for (int i = 0; i < size_plan; i++) {
      buf_beads[i] = (double *) memory->srealloc(buf_beads[i], size, "FixPATH_DYNAMICS_MB:x_beads[i]");
      buf_force[i] = (double *) memory->srealloc(
          buf_force[i], size, "FixPATH_DYNAMICS_MB:x_force[i]");    //Also allocate force buffer
    }
  }
  ...
  for (int iplan = 0; iplan < size_plan; iplan++) {
    ...
    if (nsend > max_nsend) {
      max_nsend = nsend + 200;
      tag_send =
          (int *) memory->srealloc(tag_send, sizeof(int) * max_nsend, "FixPATH_DYNAMICS_MB:tag_send");
      buf_send = (double *) memory->srealloc(buf_send, sizeof(double) * max_nsend * 3,
                                             "FixPATH_DYNAMICS_MB:x_send");
    }

    // send tags

    MPI_Sendrecv(atom->tag, nlocal, MPI_INT, plan_send[iplan], 0, tag_send, nsend, MPI_INT,
                 plan_recv[iplan], 0, uworld, MPI_STATUS_IGNORE);
    ...
    for (int i = 0; i < nsend; i++) {
      ...
      if (index < 0) {
        ...
        sprintf(error_line,
                ...
                ", " TAGINT_FORMAT ").\n",
                tag_send[i], ibead, me_w, plan_recv[iplan], atom->tag[0], atom->tag[1],
                atom->tag[2]);
    ...
    MPI_Sendrecv(buf_send, nsend * 3, MPI_DOUBLE, plan_recv[iplan], 0, buf_recv, nlocal * 3,
                 MPI_DOUBLE, plan_send[iplan], 0, uworld, MPI_STATUS_IGNORE);
...
int FixPATH_DYNAMICS_MB::pack_forward_comm(int n, int *list, double *buf, int pbc_flag, int *pbc)
{
  int i, j, m;
...
void FixPATH_DYNAMICS_MB::unpack_forward_comm(int n, int first, double *buf)
{
  int i, m, last;
...
double FixPATH_DYNAMICS_MB::memory_usage()
{
  double bytes = 0;
...
void FixPATH_DYNAMICS_MB::grow_arrays(int nmax)
{
  if (nmax == 0) return;
  int count = nmax * 3;

  memory->grow(array_atom, nmax, size_peratom_cols, "FixPATH_DYNAMICS_MB::array_atom");
  memory->grow(nhc_eta, count, nhc_nchain, "FixPATH_DYNAMICS_MB::nh_eta");
  memory->grow(nhc_eta_dot, count, nhc_nchain + 1, "FixPATH_DYNAMICS_MB::nh_eta_dot");
  memory->grow(nhc_eta_dotdot, count, nhc_nchain, "FixPATH_DYNAMICS_MB::nh_eta_dotdot");
  memory->grow(nhc_eta_mass, count, nhc_nchain, "FixPATH_DYNAMICS_MB::nh_eta_mass");
}

/* ---------------------------------------------------------------------- */

void FixPATH_DYNAMICS_MB::copy_arrays(int i, int j, int delflag)
{
  int i_pos = i * 3;
  int j_pos = j * 3;
...
int FixPATH_DYNAMICS_MB::pack_exchange(int i, double *buf)
{
  int offset = 0;
  int pos = i * 3;
...
int FixPATH_DYNAMICS_MB::unpack_exchange(int nlocal, double *buf)
{
  int offset = 0;
  int pos = nlocal * 3;
...
int FixPATH_DYNAMICS_MB::pack_restart(int i, double *buf)
{
  int offset = 0;
  int pos = i * 3;
...
void FixPATH_DYNAMICS_MB::unpack_restart(int nlocal, int nth)
{
...
int FixPATH_DYNAMICS_MB::maxsize_restart()
{
  return size_peratom_cols + 1;
}
...
int FixPATH_DYNAMICS_MB::size_restart(int nlocal)
{
  return size_peratom_cols + 1;
}
...
void FixPATH_DYNAMICS_MB::compute_force()
{
...
/*----------------------------------------------------------------------- */
void FixPATH_DYNAMICS_MB::force_clear()
{
  size_t nbytes;
...
void FixPATH_DYNAMICS_MB::compute_kinetic_energy()
{
  int nlocal = atom->nlocal;
  double **v = atom->v;
  int *type = atom->type;
  kinetic_energy = 0.0;
  for (int i = 0; i < nlocal; i++) {
    kinetic_energy += 0.5 * fmass[type[i]] *
        (v[i][0] * v[i][0] + v[i][1] * v[i][1] + v[i][2] * v[i][2]) * force->mvv2e;
  }
  double aux_E;
  aux_E = 0.0;
...
void FixPATH_DYNAMICS_MB::pimd_fill(double **ptr)
{
  comm_ptr = ptr;
...
double FixPATH_DYNAMICS_MB::compute_vector(int n)
{
...
double FixPATH_DYNAMICS_MB::compute_scalar()
{
